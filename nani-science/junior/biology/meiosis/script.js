...
const stageDetails = [
    { text: "間期（DNA複製）", n: "2n (成對)", c: "2C → 4C", desc: "【步驟0】分裂前準備：細胞進行 DNA 複製（S期）。複製完成後，遺傳物質由 2C 增為 4C（染色體數目 2n 不變，每條染色體現在由兩條姊妹染色分體組成）。" },
...
const quizData = [
    ...
    },
    {
        ...
        opts: ["(A) 兩者都會使染色體數目減半", "(B) 兩者的遺傳物質（DNA）都只有複製一次", "(C) 有絲分裂需要分裂兩次才能完成", "(D) 減數分裂最終會產生兩個子細胞"],
        a: "B",
        r: "大重點！不論是有絲分裂還是減數分裂，遺傳物質（DNA）都只在間期複製「1次」。有絲分裂只分裂 1 次產生 2 個子細胞，染色體數目不變（2n→2n）；減數分裂則分裂 2 次產生 4 個子細胞，染色體數目減半（2n→n）。"
    },
    {
